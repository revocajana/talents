...
const emptySchool = { registry_number: '', name: '', ownership_type: 'Government', country: '', zone: '', region: '', district: '', ward: '', phone: '', physical_address: '', email: '', student_count: 0 };
...
export default function TalentAdminPage() {
  ...
  const updateField = (field, value) => setSchoolForm((current) => ({ ...current, [field]: value }));

  const saveSchool = async (event) => {
    event.preventDefault(); setSchoolSubmitting(true); setError(null);
    const payload = { ...schoolForm, country: Number(schoolForm.country), zone: Number(schoolForm.zone), region: Number(schoolForm.region), district: Number(schoolForm.district), ward: Number(schoolForm.ward), student_count: Number(schoolForm.student_count || 0) };
    try {
  ...
  const locationField = (label, field, options) => <label>{label}<select required value={schoolForm[field]} onChange={(event) => updateField(field, event.target.value)}><option value="">Select {label.toLowerCase()}</option>{options.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label>;
  ...
  return <div className="sport-teacher-page talent-admin-page">
    ...
    {schoolDrawerOpen && <><button type="button" className="sport-teacher-drawer-backdrop" aria-label="Close school form" onClick={closeSchoolEditor} /><aside className="sport-teacher-search-drawer sport-teacher-registration-drawer talent-admin-school-drawer" aria-label="School editor"><div className="sport-teacher-search-drawer-header"><h2>{selectedSchool ? 'Edit school' : 'Add school'}</h2><button type="button" onClick={closeSchoolEditor} aria-label="Close school form">&times;</button></div><form onSubmit={saveSchool} className="sport-teacher-profile-form"><label>School Reg. No *<input required value={schoolForm.registry_number} onChange={(event) => updateField('registry_number', event.target.value)} maxLength="50" /></label><label>School name *<input required value={schoolForm.name} onChange={(event) => updateField('name', event.target.value)} maxLength="150" /></label><label>Ownership type<select required value={schoolForm.ownership_type} onChange={(event) => updateField('ownership_type', event.target.value)}>{ownershipTypes.map((type) => <option key={type.id} value={type.name}>{type.name}</option>)}</select></label>{locationField('Country', 'country', countries)}{locationField('Zone', 'zone', zones)}{locationField('Region', 'region', regions)}{locationField('District', 'district', districts)}{locationField('Ward', 'ward', wards)}<label>Physical address<textarea rows="3" value={schoolForm.physical_address || ''} onChange={(event) => updateField('physical_address', event.target.value)} maxLength="255" /></label><label>Phone<input value={schoolForm.phone || ''} onChange={(event) => updateField('phone', event.target.value)} maxLength="20" /></label><label>Student count<input type="number" min="0" value={schoolForm.student_count} onChange={(event) => updateField('student_count', event.target.value)} /></label><div className="talent-admin-drawer-actions">{selectedSchool && <button type="button" className="talent-admin-delete-button" onClick={deleteSchool} disabled={schoolSubmitting}>Delete</button>}<button type="submit" className="district-primary-button" disabled={schoolSubmitting}>{schoolSubmitting ? 'Saving...' : 'Save school'}</button></div></form></aside></>}
  </div>;
}
