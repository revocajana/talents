...
export default function ZoneManagerPage() {
  ...
  const zoneName = currentUser?.zone_name
    || allZones.find((zone) => Number(zone.id) === Number(selectedZone))?.name
    || 'Assigned Zone';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const regionList = regionsRes.data.results || [];
  ...
  }, [allDistricts, visibleZoneId]);

  const zoneRegions = useMemo(() => {
    if (!visibleZoneId) return regions;
    return regions.filter((region) => Number(region.zone) === visibleZoneId);
  }, [regions, visibleZoneId]);

  const zoneSchools = useMemo(() => {
    if (!visibleZoneId) return schools;
    return schools.filter((school) => Number(school.zone) === visibleZoneId);
  }, [schools, visibleZoneId]);

  const zoneWards = useMemo(() => {
    if (!visibleZoneId) return wards;
    const districtIds = new Set(zoneDistricts.map((district) => Number(district.id)));
    return wards.filter((ward) => districtIds.has(Number(ward.district)));
  }, [wards, zoneDistricts, visibleZoneId]);

  const zoneStudents = useMemo(() => {
    if (!visibleZoneId) return students;
  ...
  }, [students, zoneSchools, visibleZoneId]);

  const zoneTalents = useMemo(() => {
    if (!visibleZoneId) return studentTalents;
    const studentIds = new Set(zoneStudents.map((student) => Number(student.id)));
  ...
  const relevantAnnouncements = useMemo(() => announcements.filter((announcement) => {
    ...
    if (announcement.scope === 'zone') return Number(announcement.zone) === visibleZoneId;
    if (announcement.scope === 'region') {
      const regionIds = new Set(zoneRegions.map((region) => Number(region.id)));
      return regionIds.has(Number(announcement.region));
    }
    if (announcement.scope === 'district') {
      const districtIds = new Set(zoneDistricts.map((district) => Number(district.id)));
      return districtIds.has(Number(announcement.district));
    }
    return false;
  }), [announcements, zoneDistricts, zoneRegions, visibleZoneId]);

  const higherLevelAnnouncements = useMemo(
    () => relevantAnnouncements.filter((announcement) => ['national', 'zone', 'region'].includes(announcement.scope)),
    [relevantAnnouncements],
  );
  const lowerLevelAnnouncements = useMemo(
    () => relevantAnnouncements.filter((announcement) => announcement.scope === 'district' && Number(announcement.district) && zoneDistricts.some((district) => Number(district.id) === Number(announcement.district))),
    [relevantAnnouncements, zoneDistricts],
  );
  const homeAnnouncements = useMemo(() => relevantAnnouncements, [relevantAnnouncements]);

  const promotedTalentIds = useMemo(() => {
    const promotedDetailIds = new Set(
      promotions
  ...
  const promotedZoneTalents = useMemo(
    () => zoneTalents.filter((entry) => promotedTalentIds.has(Number(entry.id))),
    [promotedTalentIds, zoneTalents],
  );

  const talentParticipation = useMemo(() => {
    const counts = new Map();
    promotedZoneTalents.forEach((entry) => {
      const talentName = entry.talent_name || 'Other talent';
      counts.set(talentName, (counts.get(talentName) || 0) + 1);
    });
    const total = promotedZoneTalents.length;
  ...
  const schoolRanking = useMemo(
    () =>
      zoneSchools
        .map((school) => {
          const schoolStudents = zoneStudents.filter((student) => Number(student.school?.id ?? student.school) === Number(school.id));
        ...
        .sort((first, second) => second.averageScore - first.averageScore || second.participation - first.participation)
        .slice(0, 5),
    [zoneSchools, zoneStudents, participations],
  );
  ...
  const pendingPromotionStudents = useMemo(() => {
    ...
    const studentMap = new Map(zoneStudents.map((student) => [Number(student.id), student]));
    const schoolMap = new Map(zoneSchools.map((school) => [Number(school.id), school]));
  ...
  }, [participations, schoolCompetitions, zoneSchools, zoneStudents]);

  const calendarDates = useMemo(() => {
    const dates = new Set();
    zoneLevelCompetitions.forEach((competition) => {
      if (competition.start_date) dates.add(competition.start_date);
    });
    return dates;
  }, [zoneLevelCompetitions]);
  ...
  const calendarEvents = useMemo(
    () =>
      zoneLevelCompetitions
        .filter((competition) => {
          ...
          return (
            start.getFullYear() <= calendarDays.year &&
            end.getFullYear() >= calendarDays.year &&
    ...
    [calendarDays, zoneLevelCompetitions],
  );
  ...
  const handleSaveZoneScore = async (resultId, detailId, score) => {
    try {
      ...
      setError(err.response?.data?.detail || err.response?.data?.score?.[0] || 'Failed to save zone result');
    }
  };

  const handleDemoteTalent = async (resultId, detailId) => {
    if (!window.confirm('Remove this talent from zone level?')) return;
    try {
      ...
      setError(err.response?.data?.detail || err.response?.data?.error || 'Failed to remove zone promotion');
    }
  };
  ...
  const openAnnouncementEditor = (announcement = null) => {
    if (announcement && announcement.scope !== 'zone') return;
  ...
  const handleDeleteZoneAnnouncement = async () => {
    if (!selectedAnnouncement || selectedAnnouncement.scope !== 'zone' || !window.confirm('Delete this announcement?')) return;
    setAnnouncementSubmitting(true);
    try {
      ...
      <div className="district-home-grid">
        <section className="district-home-card talent-card">
          <div className="district-card-heading">
            <div>
              <h2>Talent participation</h2>
              <p>Share of talents promoted to zone level</p>
            </div>
            <span className="district-card-kicker">{promotedZoneTalents.length} promoted</span>
          </div>
          <div className="talent-chart-layout">
            <div
              className="talent-donut"
              style={{
                background: `conic-gradient(${talentParticipation
                  .map((item, index) => {
                    const palette = ['#0e1db6', '#16a085', '#f59e0b', '#e05252', '#7c3aed'];
                    const previous = talentParticipation.slice(0, index).reduce((sum, entry) => sum + entry.percentage, 0);
                    const current = talentParticipation.slice(0, index + 1).reduce((sum, entry) => sum + entry.percentage, 0);
                    return `${palette[index]} ${previous}% ${current}%`;
                  })
                  .join(', ') || '#e5e7eb 0 100%'})`,
              }}
            >
            ...
            <div className="talent-legend">
              {talentParticipation.length ? (
                talentParticipation.map((item, index) => (
                  <div className="talent-legend-row" key={item.name}>
                    <span className="legend-dot" style={{ background: ['#0e1db6', '#16a085', '#f59e0b', '#e05252', '#7c3aed'][index] }} />
                    <span>{item.name}</span>
                    <strong>{item.percentage}%</strong>
                  </div>
                ))
              ) : (
                <p>No promoted talents yet.</p>
              )}
            </div>
          </div>
        </section>
        ...
        <section className="district-home-card">
          <div className="district-card-heading">
            <div>
              ...
              <p>Read or manage zone updates</p>
            </div>
            <button type="button" className="district-text-button" onClick={() => setActiveMenu('announcements')}>View all</button>
          </div>
          <div className="recent-announcements">
            {homeAnnouncements.slice(0, 3).map((announcement) => (
              <div className="recent-announcement-row" key={announcement.id}>
                <div>
                  <strong>{announcement.title}</strong>
                  <small>{announcement.content || 'No details available.'}</small>
                </div>
              </div>
            ))}
            {!homeAnnouncements.length && <p className="district-empty-state">No active announcements yet.</p>}
          </div>
        </section>
        ...
        <section className="district-home-card">
          <div className="district-card-heading">
            <div>
              ...
              <p>Create and manage zone events</p>
            </div>
            <button type="button" className="district-primary-button district-card-action" onClick={() => openCompetitionEditor()}>Add competition</button>
          </div>
          <div className="district-competition-list">
            {zoneLevelCompetitions.map((competition) => (
            ...
            {!zoneLevelCompetitions.length && <p className="district-empty-state">No zone competitions created yet.</p>}
          </div>
          <div className="district-competition-footer">
            <span>
              {zoneLevelCompetitions.length} zone events · {schoolSubmissions.filter((item) => item.status === 'submitted').length} submissions
            </span>
            <button type="button" className="district-text-button" onClick={() => setActiveMenu('zone-results')}>Open results</button>
          </div>
        </section>
        ...
        <section className="district-home-card">
          <div className="district-card-heading">
            <div>
              ...
              <p>Ranked by average recorded score</p>
            </div>
            <span className="district-card-kicker">Top 5</span>
          </div>
          <div className="school-ranking">
            {schoolRanking.length ? (
              schoolRanking.map((school, index) => (
                <div className="school-ranking-row" key={school.name}>
                  <span className="school-rank">{index + 1}</span>
                  <div className="school-ranking-name">
                    <strong>{school.name}</strong>
                    <small>{school.students} students · {school.participation} entries</small>
                  </div>
                  <b>{school.averageScore}%</b>
                </div>
              ))
            ) : (
              <p className="district-empty-state">No school performance data yet.</p>
            )}
          </div>
        </section>
        ...
        <section className="district-home-card district-calendar-card">
          <div className="district-card-heading">
            <div>
              ...
              <p>Blue circles mark competition days</p>
            </div>
            <div className="calendar-controls">
              <button type="button" onClick={() => moveCalendarMonth(-1)} aria-label="Previous month" title="Previous month">‹</button>
              <span>{calendarDays.monthLabel}</span>
              <button type="button" onClick={() => moveCalendarMonth(1)} aria-label="Next month" title="Next month">›</button>
              <button type="button" className="calendar-today-button" onClick={() => setCalendarDate(new Date())}>Today</button>
            </div>
          </div>
          ...
          <div className="calendar-grid">
            {Array.from({ length: calendarDays.firstDay }).map((_, index) => (
              <span className="calendar-day is-empty" key={`empty-${index}`} />
            ))}
            {Array.from({ length: calendarDays.daysInMonth }, (_, index) => {
              const day = index + 1;
              const dateKey = `${calendarDays.year}-${String(calendarDays.month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
              const isToday = new Date().toISOString().slice(0, 10) === dateKey;
              return (
                <span
                  className={`calendar-day ${calendarDates.has(dateKey) ? 'has-competition' : ''} ${isToday ? 'is-today' : ''}`}
                  key={dateKey}
                  title={calendarDates.has(dateKey) ? 'Competition day' : undefined}
                >
                  {day}
                </span>
              );
          ...
          <div className="calendar-events">
            {calendarEvents.length ? (
              calendarEvents.map((competition) => (
                <div key={competition.id}>
                  <span className="calendar-event-dot" />
                  <strong>{competition.name}</strong>
                  <small>{competition.start_date}{competition.end_date ? ` - ${competition.end_date}` : ''}</small>
                </div>
              ))
            ) : (
              <p className="district-empty-state">No competitions this month.</p>
            )}
          </div>
        </section>
      </div>
  ...
  const renderResultsView = () => {
        ...
        <div className="district-results-stack">
                ...
                <table className="data-table">
                  ...
                  <tbody>
                      ...
                      <tr key={`${participation.id}-${detail.id}`}>
                        <td>
                          {isPromoted ? (
                            ...
                            <input
                              type="checkbox"
                              checked={isEligibleForPromotion && selectedPromotionStudents.includes(Number(detail.id))}
                              disabled={!isEligibleForPromotion}
                              onChange={(event) =>
                                setSelectedPromotionStudents((current) =>
                                  event.target.checked ? [...new Set([...current, Number(detail.id)])] : current.filter((id) => id !== Number(detail.id)),
                                )
                              }
                              aria-label={`Select ${student?.first_name || 'student'} ${detail.talent_name || 'talent'} for promotion`}
                            />
                          )}
                        </td>
                        ...
                        <td>
                          {detail.percentage_score >= 90
                            ? 'A+'
                            : detail.percentage_score >= 75
                              ? 'A'
                              : detail.percentage_score >= 60
                                ? 'B+'
                                : detail.percentage_score >= 50
                                  ? 'B'
                                  : detail.percentage_score >= 40
                                    ? 'C'
                                    : detail.percentage_score >= 30
                                      ? 'D'
                                      : detail.percentage_score >= 20
                                        ? 'E'
                                        : 'F'}
                        </td>
                      </tr>
                    ...
                    {!competition.entries.length && (
                      <tr>
                        <td colSpan="8" className="district-result-empty">No district-level submissions are waiting for zone review yet.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
          ...
          {!submittedDistrictCompetitions.length && (
            <section className="district-result-card">
              <p className="district-result-empty">No district competition results have been submitted yet.</p>
            </section>
          )}
        </div>
      );
    ...
    return (
      <div className="district-results-stack">
        <div className="district-results-heading">
          <div>
            <h2>Zone-level results</h2>
            <p>Promoted students are shown talent by talent. Edit and save each zone score here.</p>
          </div>
        </div>
        {zoneResultsByCompetition.map((competition) => (
          <section className="district-result-card" key={`zone-${competition.id}`}>
            <div className="district-result-card-header">
              <div>
                <h3>{competition.name}</h3>
                ...
                <tbody>
                  {competition.entries.map((result) => (
                    <tr key={`${result.id}-${result.detail?.id || 'overall'}`}>
                      ...
                      <td>{zoneSchools.find((school) => Number(school.id) === Number(result.student?.school?.id ?? result.student?.school))?.name || 'School'}</td>
                      <td>{clubMemberships.find((membership) => Number(membership.student) === Number(result.student?.id) && membership.is_active)?.club_name || '—'}</td>
                      <td>
                        <input
                          className="district-score-input"
                          type="number"
                          min="0"
                          max="100"
                      ...
                      <td>
                        {result.detail
                          ? result.detail.percentage_score === null || result.detail.percentage_score === undefined
                            ? 'Not recorded'
                            : result.detail.percentage_score >= 90
                              ? 'A+'
                              : result.detail.percentage_score >= 75
                                ? 'A'
                                : result.detail.percentage_score >= 60
                                  ? 'B+'
                                  : result.detail.percentage_score >= 50
                                    ? 'B'
                                    : result.detail.percentage_score >= 40
                                      ? 'C'
                                      : result.detail.percentage_score >= 30
                                        ? 'D'
                                        : result.detail.percentage_score >= 20
                                          ? 'E'
                                          : 'F'
                          : result.grade || 'Not recorded'}
                      </td>
                      <td>
                        {result.detail && (
                          <button type="button" className="district-demote-button" onClick={() => handleDemoteTalent(result.id, result.detail.id)}>
                            De-promote
                          </button>
                        )}
                      </td>
    ...
    <div className="district-announcements-page">
      <section className="district-announcements-card">
        <div className="district-announcements-header">
          <div>
            <h2>Higher-level announcements ({higherLevelAnnouncements.length})</h2>
            <p>Read-only announcements from national, zone, and region leadership.</p>
          </div>
        </div>
        <div className="district-announcements-list">
          {higherLevelAnnouncements.map((announcement) => (
            <article key={announcement.id} className="district-announcement-item">
              <div className="district-announcement-item-heading">
                <strong className="district-announcement-readonly-title">{announcement.title}</strong>
                <span>
                  Read only · {announcement.scope} · {announcement.published_at || announcement.created_at ? new Date(announcement.published_at || announcement.created_at).toLocaleDateString('en-GB') : 'Date unavailable'}
                </span>
              </div>
      ...
      <section className="district-announcements-card">
        <div className="district-announcements-header">
          <div>
            <h2>Zone announcements ({lowerLevelAnnouncements.length})</h2>
            <p>Announcements published by {zoneName} for the zone audience.</p>
          </div>
        ...
        <div className="district-announcements-list">
          {lowerLevelAnnouncements.map((announcement) => (
            <article key={announcement.id} className="district-announcement-item">
              <div className="district-announcement-item-heading">
                <button type="button" className="district-announcement-title" onClick={() => openAnnouncementEditor(announcement)}>{announcement.title}</button>
                <span>Zone · {announcement.published_at || announcement.created_at ? new Date(announcement.published_at || announcement.created_at).toLocaleDateString('en-GB') : 'Date unavailable'}</span>
              </div>
              <p>{announcement.content || 'No details available.'}</p>
            </article>
          ))}
          {!lowerLevelAnnouncements.length && <p className="district-empty-state">No zone announcements yet.</p>}
        </div>
      </section>
    </div>
  );
  ...
  return (
    <div className="sport-teacher-page district-manager-page">
      ...
      <aside className={`sport-teacher-navigation ${sidebarOpen ? 'is-open' : ''}`}>
        <div className="sport-teacher-navigation-heading">{zoneName}</div>
        {MENU_ITEMS.map((item) => (
          <button type="button" key={item.key} onClick={() => { setActiveMenu(item.key); setSidebarOpen(false); }}>
            {item.label}
          </button>
        ))}
      </aside>
      ...
      <main className="sport-teacher-prototype-content district-manager-content">
        ...
        {loading ? (
          <DashboardSkeleton label="Loading zone manager dashboard" />
        ) : (
          <>
      ...
      {competitionDrawerOpen && (
        <>
          ...
          <aside className="sport-teacher-search-drawer sport-teacher-registration-drawer district-competition-drawer" style={{ '--drawer-width': `${competitionDrawerWidth}px` }} aria-label="Zone competition form">
            ...
            <div className="sport-teacher-search-drawer-header">
              ...
              <button type="button" onClick={closeCompetitionEditor} aria-label="Close competition form">&times;</button>
            </div>
            <form onSubmit={handleSaveCompetition} className="sport-teacher-profile-form">
              <p className="district-drawer-note">This event will be visible to schools in {zoneName}.</p>
              <label>
                Competition name *
                <input type="text" value={competitionForm.name} placeholder="e.g. Zone Athletics Cup" onChange={(event) => setCompetitionForm({ ...competitionForm, name: event.target.value })} maxLength="150" required />
              </label>
              <label>
                Description
                <textarea value={competitionForm.description} placeholder="Optional competition summary..." onChange={(event) => setCompetitionForm({ ...competitionForm, description: event.target.value })} rows="5" />
              </label>
              <div className="district-competition-date-grid">
                <label>
                  Start date *
                  <input type="date" value={competitionForm.start_date} onChange={(event) => setCompetitionForm({ ...competitionForm, start_date: event.target.value })} required />
                </label>
                <label>
                  End date
                  <input type="date" value={competitionForm.end_date} onChange={(event) => setCompetitionForm({ ...competitionForm, end_date: event.target.value })} min={competitionForm.start_date || undefined} />
                </label>
              </div>
              <div className="district-competition-form-actions">
                <label>
                  Status
                  <select value={competitionForm.status} onChange={(event) => setCompetitionForm({ ...competitionForm, status: event.target.value })}>
                ...
                {selectedCompetition && (
                  <button type="button" className="district-competition-delete" onClick={handleDeleteCompetition} disabled={competitionSubmitting}>Delete</button>
                )}
                <button type="submit" className="district-primary-button" disabled={competitionSubmitting}>
                  {competitionSubmitting ? 'Saving...' : selectedCompetition ? 'Save changes' : 'Create competition'}
                </button>
              </div>
            </form>
          </aside>
      ...
      {announcementDrawerOpen && (
        <>
          ...
          <aside className="sport-teacher-search-drawer sport-teacher-registration-drawer district-competition-drawer" style={{ '--drawer-width': `${competitionDrawerWidth}px` }} aria-label="Zone announcement form">
            ...
            <div className="sport-teacher-search-drawer-header">
              ...
              <button type="button" onClick={() => setAnnouncementDrawerOpen(false)} aria-label="Close announcement form">&times;</button>
            </div>
            <form onSubmit={handleCreateAnnouncement}>
              <div className="sport-teacher-announcement-form district-announcement-drawer-form">
                <label>
                  Title *
                  <input type="text" value={announcementForm.title} placeholder="e.g. Zone sports meeting" onChange={(event) => setAnnouncementForm({ ...announcementForm, title: event.target.value })} maxLength="200" required />
                </label>
                <label>
                  Message *
                  <textarea value={announcementForm.content} placeholder="Type the announcement details here..." onChange={(event) => setAnnouncementForm({ ...announcementForm, content: event.target.value })} rows="7" required />
                </label>
                <div className="sport-teacher-announcement-form-actions">
                  <label>
                    Expires on (optional)
                    <input type="date" value={announcementForm.expires_at} onChange={(event) => setAnnouncementForm({ ...announcementForm, expires_at: event.target.value })} />
                  </label>
                  {selectedAnnouncement && (
                    <button type="button" className="sport-teacher-announcement-delete" onClick={handleDeleteZoneAnnouncement} disabled={announcementSubmitting}>Delete</button>
                  )}
      ...
      {profileDrawerOpen && (
        <>
          ...
          <aside className="sport-teacher-search-drawer sport-teacher-registration-drawer district-competition-drawer" style={{ '--drawer-width': `${competitionDrawerWidth}px` }} aria-label="Profile details">
            ...
            <form onSubmit={handleSaveProfile} className="sport-teacher-profile-form">
              <div className="sport-teacher-profile-details">
                <div><span>Username:</span><strong>{currentUser?.username || '-'}</strong></div>
                <div><span>Firstname:</span><strong>{currentUser?.first_name || '-'}</strong></div>
                <div><span>Lastname:</span><strong>{currentUser?.last_name || '-'}</strong></div>
                <div><span>Role:</span><strong>{currentUser?.role || '-'}</strong></div>
                <div><span>Zone:</span><strong>{zoneName}</strong></div>
              </div>
              <label>
                Email
                <input type="email" value={profileEmail} onChange={(event) => setProfileEmail(event.target.value)} />
              </label>
              {profileMessage && <div className={`district-profile-message is-${profileMessage.type}`} role="status">{profileMessage.text}</div>}
            ...
            <form onSubmit={handleChangePassword} className="sport-teacher-profile-form">
              <label>
                ...
                <span className="sport-teacher-password-control">
                  <input type={showProfilePassword ? 'text' : 'password'} value={profilePasswordForm.new_password} onChange={(event) => setProfilePasswordForm({ ...profilePasswordForm, new_password: event.target.value })} minLength="8" required />
                  <button type="button" onClick={() => setShowProfilePassword((visible) => !visible)} aria-label={showProfilePassword ? 'Hide new password' : 'Show new password'}>
                    <EyeIcon visible={showProfilePassword} />
                  </button>
                </span>
              </label>
              <label>
                Confirm password
                <span className="sport-teacher-password-control">
                  <input type={showProfilePasswordConfirmation ? 'text' : 'password'} value={profilePasswordForm.confirm_password} onChange={(event) => setProfilePasswordForm({ ...profilePasswordForm, confirm_password: event.target.value })} minLength="8" required />
                  <button type="button" onClick={() => setShowProfilePasswordConfirmation((visible) => !visible)} aria-label={showProfilePasswordConfirmation ? 'Hide password confirmation' : 'Show password confirmation'}>
                    <EyeIcon visible={showProfilePasswordConfirmation} />
                  </button>
                </span>
              </label>
